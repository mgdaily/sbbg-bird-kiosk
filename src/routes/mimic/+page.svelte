<script lang="ts">
    import { appState, getSelectedBirdMetadata, getSelectedAudioPath } from '$lib/stores/appStore.svelte';
...
<div class="min-h-screen flex flex-col p-8">
    ...
    <div class="flex-1 flex flex-col items-center justify-center">
        <h1 class="headline text-5xl md:text-6xl text-center mb-4">{metadata?.name || 'Bird'}</h1>
        ...
        <div class="flex gap-4 mb-8">
            <button
                ...
                class="px-6 py-3 rounded transition-colors {audioType === 'call' ? 'bg-white/30 text-white border-2 border-white' : 'bg-white/10 text-white/70 border-2 border-white/30 hover:bg-white/20'}"
            >
                Call
            </button>
            <button
                ...
                class="px-6 py-3 rounded transition-colors {audioType === 'song' ? 'bg-white/30 text-white border-2 border-white' : 'bg-white/10 text-white/70 border-2 border-white/30 hover:bg-white/20'}"
            >
                Song
            </button>
        </div>
